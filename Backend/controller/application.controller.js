...
export const applyJob = async (req, res)=> {
    try {
        ...
        if(existingApplication){
            res.status(400).json({message:"You have already applied for this job", success:false})
        }
...
export const getAppliedJobs = async (req, res)=>{
    try {
        ...
        populate({
            ...
            papulate:{path:"company", options:{sort:{createdAt:-1}}}
        });
        if(!applications){
            res.status(404).json({message:"Application Not Found", success:false});
        }
        
        res.status(200).json({applications, success:false});
...
export const updateStatus = async (req, res)=> {
    try {
        ...
        if(!application){
            return res.status(404).json({message:"Apllication not found", success:false});
        }
